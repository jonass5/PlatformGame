...
func test_should_return_true_when_player_activated_missiles():
	# arrange
	PlayerStats.set_max_missiles(1)

	# act
	var result = PlayerStats.is_missiles_activated()

	# assert
	assert_true(result)


func test_should_return_false_when_player_has_not_activated_missiles():
	# arrange
	PlayerStats.set_max_missiles(0)
